...
/**
 * This program used to collect the Stock Indices value from Bloomberg. Taking Stock Index list from config.property
 * and output the index information to /embers/data-collector. 
 * <p>
 * The output format would be like as
 * stockIndex current_value previous_close open_value update_time insert_time
 * <p>
 * each record for one line and each day's data would be stored in a individual file named as stock_yyyy_mm_dd.txt
 * @author wei
 * @version 2012-7-10
 */
...
public class StockIndexDataCollector {
	
	private Properties pro;
	private BufferedWriter bw;
	private BufferedReader br;
	
	public StockIndexDataCollector()
	{
		pro = new Properties();
		try{
			...
			String stockIndexList = pro.getProperty("stockIndexList");
			...
			String outputName = outputPath + "/stock_" + currentDay + ".txt";
	...
	public void scrapeStockIndex() throws IOException
	{
		...
		while((stockIndex=br.readLine())!=null)
		{
			...
			String url = "http://www.bloomberg.com/quote/"+stockIndex+":IND";
	...
	public String getIndexQuote(String stockIndex,String url) throws IOException
	{
		...
		Elements elements = doc.getElementsByClass("ticker_header");
		for(Element el:elements)
		{
			...
			{
				currentValue = element.text().trim().replaceAll(",", "");
			}
	...
	public static void main(String[] args) {
		...
		StockIndexDataCollector sidc = new StockIndexDataCollector();
		try{
			sidc.scrapeStockIndex();
			sidc.close();
		}
		catch(IOException ex)
		{
